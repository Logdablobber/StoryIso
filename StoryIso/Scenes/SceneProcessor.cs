...
public static class SceneProcessor
{
	public static Scene? ProcessScene(string path)
	{
		...
		List<Function>? functions = FunctionProcessor.Process(path, scene_text, 1);
		...
		return new Scene(name, functions);
	}

	public static void PreprocessScene(string path)
	{
		string scene_text;
		using(StreamReader streamReader = new StreamReader(path))
		{
		...
		FunctionProcessor.Preprocess(path, scene_text, 1);
	}
}