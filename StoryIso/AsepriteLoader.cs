...
public static class AsepriteLoader
{
	...
	public static void LoadAsefiles(GraphicsDevice graphics, string path)
	{
		...
		foreach (string f in files)
		{
			...
			MonoGame.Aseprite.SpriteSheet spriteSheet = asefile.CreateSpriteSheet(graphics, true);
	...
	private static AsepriteFile LoadAsefile(string path)
	{
		...
		{
			aseFile = AsepriteFileLoader.FromStream(fileName: path, stream: stream, preMultiplyAlpha: false);
		}
