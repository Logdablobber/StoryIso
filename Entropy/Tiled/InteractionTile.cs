...
public class InteractionTile
{
	...
	private void RunInteraction()
	{
		...
		if (!interactingLastFrame)
		{
			if (onInteract != null)
			{
				ScopeProcessor.RunScope(onInteract, $"onInteract of Interaction {id}", new Source(0, null, $"onInteract of Interaction {id}"), sync: true);
			}
			
			if (toggleState && onToggleOff != null)
			{
				ScopeProcessor.RunScope(onToggleOff, $"onToggleOff of Interaction {id}", new Source(0, null, $"onToggleOff of Interaction {id}"), sync: true);
			}
			else if (!toggleState && onToggleOn != null)
			{
				ScopeProcessor.RunScope(onToggleOn, $"onToggleOn of Interaction {id}", new Source(0, null, $"onToggleOn of Interaction {id}"), sync: true);
			}
