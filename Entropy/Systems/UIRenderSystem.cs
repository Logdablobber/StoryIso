...
public class UIRenderSystem : EntityDrawSystem
{
	...
	public override void Draw(GameTime gameTime)
	{
		...
		_spriteBatch.Begin(samplerState: SamplerState.PointClamp, sortMode: SpriteSortMode.Deferred, blendState: BlendState.NonPremultiplied);
		...
		foreach (var entityId in ActiveEntities)
		{
			...
			GetScreenTransform(info.Position, info.Scale,
			...
			{                
				...
				animation.GetFrame().Draw(_spriteBatch, draw_position, render_attributes.ColorWithOpacity, 0, origin, draw_scale, SpriteEffects.None, render_attributes.renderLayer.GetLayerDepth());
				continue;
			}
			...
			if (texture != null)
			{
				...
				_spriteBatch.Draw(texture, draw_position, null, render_attributes.ColorWithOpacity, 0, origin, draw_scale, SpriteEffects.None, render_attributes.renderLayer.GetLayerDepth());
				continue;
			}
			...
			{
				// TODO: implement origin
				rect.Draw(_spriteBatch, render_attributes.ColorWithOpacity, draw_position, draw_scale, render_attributes.renderLayer.GetLayerDepth());
				continue;
			}
			...
			if (polygon != null)
			{
				polygon.Draw(render_attributes.ColorWithOpacity, info.Position, info.Scale, render_attributes.renderLayer.GetLayerDepth());
				continue;
			}
	...
	public static void GetScreenTransform(Vector2 position, 
                                    Vector2 scale,
									...
									out Vector2 screen_scale)
	{
		if (position.X < 0)
		{
			position.X = Game1.ViewportAdapter.VirtualWidth + position.X;
		}

		if (position.Y < 0)
		{
			position.Y = Game1.ViewportAdapter.VirtualHeight + position.Y;
		}

		screen_position = position * Game1.ScreenScale;
		screen_scale = scale * Game1.ScreenScale;
	}
